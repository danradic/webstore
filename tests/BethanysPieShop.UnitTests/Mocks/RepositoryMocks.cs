...
namespace BethanysPieShop.UnitTests.Mocks
{
    public class RepositoryMocks
    {
        public static Mock<IPieRepository> GetPieRepositoryMock() 
        {
            ...
            var pieRepositoryMock = new Mock<IPieRepository>();
            pieRepositoryMock.Setup(repo => repo.AllPies).Returns(pies);
            pieRepositoryMock.Setup(repo => repo.PiesOfTheWeek).Returns(pies.Where(p => p.IsPieOfTheWeek));
            pieRepositoryMock.Setup(repo => repo.GetPieById(It.IsAny<int>())).Returns(pies.FirstOrDefault());

            return pieRepositoryMock;
        }

        public static Mock<ICategoryRepository> GetPCatergoryRepositoryMock()
        {
            ...
            categoryRepositoryMock.Setup(m => m.AllCategories).Returns(categories);
